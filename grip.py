...
#grip handle gripping calcs etx
# .ttt
...
with Client() as client:
    ...
    def demoGRIPs():
        print ("GRIP")
        for i in np.arange(0,1.5,.05):
            sim.simxSetJointPosition(client.id,griph,degToRad((i)),sim.simx_opmode_blocking)
             
            c,pos = sim.simxGetJointPosition(client.id,griph,sim.simx_opmode_blocking)
            print(i,degToRad(i), radToDeg(pos))
        print ("DONE GRIP")

        print ("GRIPm")
        for i in np.arange(0,1.5,.05):
            sim.simxSetJointPosition(client.id,griphmh,degToRad((i)),sim.simx_opmode_blocking)
             
            c,pos = sim.simxGetJointPosition(client.id,griphmh,sim.simx_opmode_blocking)
            print(i,degToRad(i) , radToDeg(pos))
        print ("DONE GRIP")
        sim.simxSetJointPosition(client.id,griphmh,-2,sim.simx_opmode_blocking)
             
        c,pos = sim.simxGetJointPosition(client.id,griphmh,sim.simx_opmode_blocking)
        print(pos , radToDeg(pos))
    ...
    if client.id!=-1:
        ...
        while i<8:
            ...
            joints[s]["handle"] = h
            i+=1 
        print ("Done Configuringg")

        movAll([0,0,-90,0,-45,30,0])
        demoGRIPs()
